...
namespace Misakai.Kafka
{
    public abstract class KafkaRequest
    {
        ...
        public static void EncodeHeader<T>(BinaryStream writer, IKafkaRequest<T> request)
        {
            writer.Write(request.ApiKey);
            writer.Write(ApiVersion);
            writer.Write(request.CorrelationId);
